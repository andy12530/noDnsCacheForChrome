...
def ReadDataPack(input_file):
  ...
  for _ in range(num_entries):
    ...
    if id == 1501:
      ### load file, the new\\1501.bin is the file that you want. 
      of = open("new\\"+'{0}.{1}'.format(id, 'bin'),'wb')  
      of.write(original_data[offset:next_offset])
      of.close()
      return

      ### edit the file 1501.bin, rePack will instend old file.
      # of = open("new\\1501.bin",'rb')  
      # tmpdata = of.read()
      # of.close()
      # resources[id] = tmpdata
      ### notice!!! After edit file, the length must be same the old file.
      # print len(tmpdata)
    else:
      resources[id] = original_data[offset:next_offset]
  return DataPackContents(resources, encoding)
...
def WriteDataPackToString(resources, encoding):
  ...
  for id in ids:
    ret.append(struct.pack("<HI", int(id), int(data_offset) ))
...
def RePack(output_file, input_files):
  ...
  new_content = ReadDataPack(input_files)
...
def main():
  if len(sys.argv) > 1:
    RePack("resources.pak", sys.argv[1])
